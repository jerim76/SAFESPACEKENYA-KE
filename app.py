...
st.markdown("<div style='text-align: center; padding-bottom: 15px; margin-bottom: 30px;'><h2 style='color: #1E3A5F; font-size: 2.2rem; font-weight: 600;'>We're Here for You</h2><div style='width: 100px; height: 4px; background: #26A69A; margin: 0 auto; border-radius: 2px;'></div></div>", unsafe_allow_html=True)
st.markdown("""
...
try:
    map_data = pd.DataFrame({'lat': [-1.2985], 'lon': [36.7848]})  # Precise coordinates for Greenhouse Plaza
    st.map(map_data, zoom=14, use_container_width=True)
except Exception as e:
    st.markdown("<p style='color: #FF6F61; font-size: 1rem;'>Unable to display map. Please check your internet connection or visit our center at Greenhouse Plaza, Ngong Road, Nairobi.</p>", unsafe_allow_html=True)

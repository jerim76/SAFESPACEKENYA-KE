...
for i, service in enumerate(services_content):
    if i % 3 == 0:
        cols = st.columns(3)
...
st.markdown("</div>", unsafe_allow_html=True)  # Close services section
...
# Function to load team photos
def load_team_photo(index):
    # Using placeholder images for demo
    photo_urls = [
        "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?crop=entropy&cs=tinysrgb&fit=crop&fm=jpg&h=200&w=200",
        "https://images.unsplash.com/photo-1560250097-0b93528c311a?crop=entropy&cs=tinysrgb&fit=crop&fm=jpg&h=200&w=200",
        "https://images.unsplash.com/photo-1590086782792-42dd2350140d?crop=entropy&cs=tinysrgb&fit=crop&fm=jpg&h=200&w=200",
        "https://images.unsplash.com/photo-1564564321837-a57b7070ac4f?crop=entropy&cs=tinysrgb&fit=crop&fm=jpg&h=200&w=200"
    ]
    try:
        response = requests.get(photo_urls[index])
        return Image.open(BytesIO(response.content))
    except:
        return Image.new('RGB', (200, 200), color='#E8F4F8')

for i, col in enumerate(team_cols):
    if i < len(team_members):
        with col:
            member = team_members[i]
            team_photo = load_team_photo(i)
            st.image(team_photo, width=200, caption=member['name'])
            st.markdown(f"""
...
st.markdown("</div>", unsafe_allow_html=True)  # Close team section
...
for i, testimonial in enumerate(testimonials_content):
    with testimonial_cols[i % 2]:
        st.markdown(f"""
...
st.markdown("</div>", unsafe_allow_html=True)  # Close testimonials section
...
st.markdown("</div>", unsafe_allow_html=True)  # Close contact section

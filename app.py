...
knowledge_base = [
    ...
    {"question": r"what services do you offer\??", "answer": "We provide individual, group, and online counseling. Book an appointment below."},
    ...
    {"default": f"We’re here to help. Ask about services, contact, hours, costs, founders, events, volunteering, crisis support, or partnerships. Time: 12:16 PM EAT, August 9, 2025."}
]
...
# SERVICES SECTION with Appointment Booking
...
with st.form("appointment_form", clear_on_submit=True):
    ...
    time = st.time_input("Preferred Time", value=datetime.strptime("10:00", "%H:%M").time())
    submit = st.form_submit_button("Book Appointment")
    if submit and name and re.match(r"[^@]+@[^@]+\.[^@]+", email):
        appointment = {"name": name, "email": email, "date": date, "time": time, "booked_at": datetime.now()}
        st.session_state.appointments.append(appointment)
        st.success(f"Thank you, {name}! Your appointment is booked at 12:16 PM EAT, August 9, 2025, for {date.strftime('%Y-%m-%d')} at {time.strftime('%H:%M')}. We’ll confirm via {email}.")
    elif submit:
...
if st.button("Save"):
    ...
    st.success(f"Saved at 12:16 PM EAT, August 9, 2025. Take care!")
...
with st.form("volunteer_form", clear_on_submit=True):
    ...
    if submit and name and re.match(r"[^@]+@[^@]+\.[^@]+", email):
        ...
        st.success(f"Thank you, {name}! Contact at 12:16 PM EAT, August 9, 2025, via {email}.")
...
with st.form("feedback_form", clear_on_submit=True):
    ...
    if submit and feedback:
        ...
        st.success(f"Thank you! Noted at 12:16 PM EAT, August 9, 2025.")
    elif submit:
...
2. **Test Locally**:
   - Run `streamlit run app.py` and verify the appointment booking works with date/time selection.
3. **Set Up GitHub Repository**:
   - `git init`, `git add app.py requirements.txt`, `git commit -m "SafeSpace app with appointment booking"`.
...
- **Form**: Includes `st.date_input` for the preferred date (minimum today) and `st.time_input` for the preferred time, using `datetime` for validation and storage.
- **Storage**: Appointments are saved in `session_state.appointments` with fields `name`, `email`, `date`, `time`, and `booked_at`.
- **Confirmation**: Displays a success message with the booked date and time, updated to 12:16 PM EAT, August 9, 2025.

This version integrates appointment booking, fits within the screen, and remains user-friendly and deployable. Test locally and let me know if further tweaks are needed!

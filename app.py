...
# Initialize session state for chatbot, progress tracker, and volunteer forms
...
with st.form("outreach_form", clear_on_submit=True):
    ...
    experience = st.text_area("Relevant Experience (e.g., community work, public speaking)", placeholder="Describe your experience")
    skills = st.multiselect("Skills", ["Communication", "Public Speaking", "Event Planning", "Community Engagement"])
...
with st.form("event_form", clear_on_submit=True):
    ...
    email = st.text_input("Email", placeholder="your.email@example.com")
    phone = st.text_input("Phone Number", placeholder="+254 XXX XXX XXX")
    experience = st.text_area("Relevant Experience (e.g., event management, logistics)", placeholder="Describe your experience")
    skills = st.multiselect("Skills", ["Event Coordination", "Logistics", "Teamwork", "Customer Service"])
    submit_event = st.form_submit_button("Submit Application")

# Handle form submissions outside the form blocks
if submit_outreach:
    if not name or not email or not re.match(r"[^@]+@[^@]+\.[^@]+", email) or not phone or not experience:
        st.error("Please fill in all required fields.")
    else:
        st.session_state.outreach_form = {"name": name, "email": email, "phone": phone, "experience": experience, "skills": skills}
...
if submit_event:
    if not name or not email or not re.match(r"[^@]+@[^@]+\.[^@]+", email) or not phone or not experience:
        ...
        st.session_state.event_form = {"name": name, "email": email, "phone": phone, "experience": experience, "skills": skills}
        st.success("Thank you for applying! We'll review your application and contact you soon.")

...
<div class='chatbot-container' id='chatbot'>
    <div id='chat-messages'>
        """ + "".join([f"<div class='chatbot-message {'user' if m.get('role') == 'user' else 'bot'}'>{m.get('content', '')}</div>" for m in st.session_state.chat_history]) + """
    </div>
...
<script>
        ...
        chatInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter' && this.value.trim()) {
                ...
                if (chatMessages) {
                    ...
                    // Simulate chatbot response (since no backend is implemented here)
                    const response = "This is a simulated response. " + get_chatbot_response(message);
    ...
    function get_chatbot_response(query) {
        // This is a

...
knowledge_base = [
    ...
    {"default": "I’m sorry, I didn’t understand. Ask about services, contact, hours, costs, founders, events, volunteering, crisis support, or partnerships, or visit Contact. Time: 12:57 AM EAT, August 06, 2025."}
]
...
with st.form("counseling_form", clear_on_submit=True):
    ...
    if submit:
        if not all([name, email, re.match(r"[^@]+@[^@]+\.[^@]+", email), phone]):
            ...
            st.success(f"Thank you, {name}! Your {counseling_type} counseling registration at 12:57 AM EAT, August 06, 2025, is received. Contact at {email} within 48 hours.")
            st.session_state.counseling_form_data = {"name": "", "email": "", "phone": "", "type": "Online"}
st.markdown("""
...
if st.button("Log Mood"):
    ...
    st.success(f"Logged at 12:57 AM EAT, August 06, 2025!")
...
with st.form("volunteer_form", clear_on_submit=True):
    ...
    if submit:
        if not all([name, email, re.match(r"[^@]+@[^@]+\.[^@]+", email), phone, experience]):
            ...
            st.success(f"Thank you, {name}! Registered at 12:57 AM EAT, August 06, 2025. Contact at {email} within 48 hours.")
...
with st.form("partnership_form", clear_on_submit=True):
    ...
    if submit:
        if not all([name, email, re.match(r"[^@]+@[^@]+\.[^@]+", email), phone]):
            ...
            st.success(f"Thank you, {name}! Your {partnership_type} registration at 12:57 AM EAT, August 06, 2025, is received. Contact at {email} within 48 hours.")
            st.session_state.partnership_form_data = {"name": "", "organization": "", "email": "", "phone": "", "type": "Partner"}
        ...
        if (toggleButton && chatbot && chatInput && chatMessages) {
            ...
            chatInput.addEventListener('keypress', function(e) {
                if (e.key === 'Enter' && this.value.trim()) {
                    ...
                    const response = "This is a simulated response. " + getChatbotResponse(message);

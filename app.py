...
knowledge_base = [
    ...
    {"default": "I’m sorry, I didn’t understand. Ask about services, contact, hours, costs, founders, events, volunteering, crisis support, partnerships, or resources, or visit Contact. Time: 09:03 PM EAT, August 05, 2025."}
]
...
with st.form("counseling_form", clear_on_submit=True):
    ...
    if submit:
        if not all([name, email, re.match(r"[^@]+@[^@]+\.[^@]+", email), phone]):
            ...
            st.success(f"Thank you, {name}! Your {counseling_type} counseling registration at 09:03 PM EAT, August 05, 2025, has been received. We will contact you soon.")
...
with st.form("appointment_form", clear_on_submit=True):
    ...
    if submit:
        if not all([name, email, re.match(r"[^@]+@[^@]+\.[^@]+", email), phone, date, time]):
            ...
            st.success(f"Thank you, {name}! Your {appointment_type} appointment on {date.strftime('%Y-%m-%d')} at {time.strftime('%H:%M')} EAT has been booked at 09:03 PM EAT, August 05, 2025. We will confirm soon.")

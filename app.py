...
# ---------- Layout: Multi-tabs ----------
tabs = st.tabs(["Home", "Therapy & Counseling", "Education", "Tools & Activities", "Founders", "Therapists", "Contact & Support", "FAQ & Resources"])
...
with tabs[0]:
    ...
    st.markdown("<h1 style='color: white; font-size: 2.4rem; margin-bottom: 0.3rem;'>SafeSpace Counseling</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: rgba(255,255,255,0.95); font-size: 1.1rem; max-width: 900px; margin: 0 auto;'>A compassionate, evidence-based mental health service supporting individuals, couples and families. Confidential care, culturally-informed, and accessible resources for emotional wellbeing.</p>", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    st.markdown("## Therapy & Counseling Services")
    st.markdown("We offer individualized plans, group sessions, workshops, and crisis support. All therapists are credentialed and engage in ongoing supervision and training.")
    ...
    # Intake form
    st.markmark = st.markdown  # keep linter happy if referenced
    st.markdown("<div class='safe-container'>", unsafe_allow_html=True)
...
with tabs[4]:
    ...
    st.markdown(textwrap.dedent("""
        Jerim Owino is the visionary behind SafeSpace Counseling. With a background in community mental health, education, and counseling practice, Jerim established SafeSpace to improve access to culturally-informed therapy, training, and advocacy across Kenya.
...
with tabs[7]:
    ...
    st.markdown("- The Body Keeps the Score — Bessel van der Kolk")
    st.markdown("- Feeling Good — David D. Burns")
    st.markdown("- Self-Compassion — Kristin Neff")
    st.markdown("---")
...
st.markdown(
    ...
    <div style='margin-top: 2rem; padding: 1.6rem; text-align: center; background: #e8f4f8; border-top: 1px solid #d1e7e3;'>
        <div style='max-width:900px; margin: 0 auto;'>
            <strong>SafeSpace Counseling</strong> — Professional, confidential mental health care for individuals, couples, and families.
            <div style='margin-top: 0.6rem; font-size: 0.9rem; color: #6c757d;'>Contact: +254 781 095 919 • info@safespace.org</div>
            <div style='margin-top: 0.6rem; font-size: 0.85rem; color: #6c757d;'>© 2025 SafeSpace Counseling • Licensed & Confidential</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,

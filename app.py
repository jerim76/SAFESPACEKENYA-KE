...
if "outreach_form" not in st.session_state:
    st.session_state.outreach_form = {}
...
<div style='text-align: center; padding: 1rem; background: var(--primary); color: white;'>
    ...
    <button id='chatbot-toggle' class='primary-btn' style='margin-top: 1rem;'>Toggle Chatbot</button>
</div>
""", unsafe_allow_html=True)
...
# Initialize form state outside the form blocks
if "outreach_name" not in st.session_state:
    st.session_state.outreach_name = ""
if "outreach_email" not in st.session_state:
    st.session_state.outreach_email = ""
if "outreach_phone" not in st.session_state:
    st.session_state.outreach_phone = ""
if "outreach_experience" not in st.session_state:
    st.session_state.outreach_experience = ""
if "outreach_skills" not in st.session_state:
    st.session_state.outreach_skills = []

if "event_name" not in st.session_state:
    st.session_state.event_name = ""
if "event_email" not in st.session_state:
    st.session_state.event_email = ""
if "event_phone" not in st.session_state:
    st.session_state.event_phone = ""
if "event_experience" not in st.session_state:
    st.session_state.event_experience = ""
if "event_skills" not in st.session_state:
    st.session_state.event_skills = []

# Outreach Support Application Form (moved after Volunteer Opportunities)
st.markdown("<h3>Apply for Outreach Support</h3>", unsafe_allow_html=True)
with st.form("outreach_form", clear_on_submit=True):
    st.session_state.outreach_name = st.text_input("Full Name", value=st.session_state.outreach_name, placeholder="Your full name")
    st.session_state.outreach_email = st.text_input("Email", value=st.session_state.outreach_email, placeholder="your.email@example.com")
    st.session_state.outreach_phone = st.text_input("Phone Number", value=st.session_state.outreach_phone, placeholder="+254 XXX XXX XXX")
    st.session_state.outreach_experience = st.text_area("Relevant Experience (e.g., community work, public speaking)", value=st.session_state.outreach_experience, placeholder="Describe your experience")
    st.session_state.outreach_skills = st.multiselect("Skills", ["Communication", "Public Speaking", "Event Planning", "Community Engagement"], default=st.session_state.outreach_skills)
    submit_outreach = st.form_submit_button("Submit Application")

# Event Volunteer Application Form (moved after Events section)
st.markdown("</div>", unsafe_allow_html=True)
...
with st.form("event_form", clear_on_submit=True):
    st.session_state.event_name = st.text_input("Full Name", value=st.session_state.event_name, placeholder="Your full name")
    st.session_state.event_email = st.text_input("Email", value=st.session_state.event_email, placeholder="your.email@example.com")
    st.session_state.event_phone = st.text_input("Phone Number", value=st.session_state.event_phone, placeholder="+254 XXX XXX XXX")
    st.session_state.event_experience = st.text_area("Relevant Experience (e.g., event management, logistics)", value=st.session_state.event_experience, placeholder="Describe your experience")
    st.session_state.event_skills = st.multiselect("Skills", ["Event Coordination", "Logistics", "Teamwork", "Customer Service"], default=st.session_state.event_skills)
...
if submit_outreach:
    if not st.session_state.outreach_name or not st.session_state.outreach_email or not re.match(r"[^@]+@[^@]+\.[^@]+", st.session_state.outreach_email) or not st.session_state.outreach_phone or not st.session_state.outreach_experience:
        st.error("Please fill in all required fields.")
    else:
        st.session_state.outreach_form = {
            "name": st.session_state.outreach_name,
            "email": st.session_state.outreach_email,
            "phone": st.session_state.outreach_phone,
            "experience": st.session_state.outreach_experience,
            "skills": st.session_state.outreach_skills
        }
...
if submit_event:
    if not st.session_state.event_name or not st.session_state.event_email or not re.match(r"[^@]+@[^@]+\.[^@]+", st.session_state.event_email) or not st.session_state.event_phone or not st.session_state.event_experience:
        st.error("Please fill in all required fields.")
    else:
        st.session_state.event_form = {
            "name": st.session_state.event_name,
            "email": st.session_state.event_email,
            "phone": st.session_state.event_phone,
            "experience": st.session_state.event_experience,
            "skills": st.session_state.event_skills
        }
        st.success("Thank you for applying! We'll review your application and contact you soon.")
        # Clear form fields after successful submission
        st.session_state.event_name = ""

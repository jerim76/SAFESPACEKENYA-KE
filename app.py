...
<style>
    ...
    h2 { font-size: 2.2rem; }
    h3 { font-size: 1.8rem; }
    .service-card, .team-card, .testimonial-card, .cta-banner, .insights-content, .chatbot-container, .resource-card, .story-card, .event-card, .partnership-card {
        background: white;
        padding: 1.5rem;
        border-radius: 12px;
    ...
    .service-card:hover, .team-card:hover, .testimonial-card:hover, .resource-card:hover, .story-card:hover, .event-card:hover, .partnership-card:hover {
    ...
    @media (max-width: 768px) {
        ...
        .service-card, .team-card, .testimonial-card, .insights-content, .chatbot-container, .resource-card, .story-card, .event-card, .partnership-card {
            margin: 0.5rem;
        }
        .chatbot-container {
            width: 90%;
...
resources = [
    ...
    {
        "title": "Mindfulness Audio",
        "desc": "A short audio guide for practicing mindfulness in daily life."
    }
]
cols = st.columns(3)
for i, resource in enumerate(resources):
    with cols[i % 3]:
        ...
        <div class='resource-card'>
            ...
            <p>{resource['desc']}</p>
            <a href='#' class='primary-btn' style='display: block; text-align: center; margin-top: 1rem;'>Download</a>
        </div>
        """, unsafe_allow_html=True)
...
knowledge_base = [
    ...
    {
        "section": "Resources",
        "answer": "Access free resources like breathing exercise guides, journaling prompts, and mindfulness audio to support your mental wellbeing.",

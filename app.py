...
with st.form("volunteer_form", clear_on_submit=True):
    ...
    experience = st.text_area("Relevant Experience", placeholder="Describe your experience")
    submit = st.form_submit_button("Register")
    if submit:
        if not name or not email or not re.match(r"[^@]+@[^@]+\.[^@]+", email) or not phone or not experience:
            st.error("Please fill in all fields.")
        else:
            st.session_state.outreach_form_data = {"name": name, "email": email, "phone": phone, "experience": experience, "skills": []}

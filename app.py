...
<style>
    ...
    h2 { font-size: 2.2rem; }
    h3 { font-size: 1.8rem; }
    .service-card, .team-card, .testimonial-card, .cta-banner, .insights-content, .chatbot-message {
        background: white;
        padding: 1.5rem;
        border-radius: 12px;
    ...
    html {
        scroll-behavior: smoothទ0smooth;
    }
...
# Chatbot in Sidebar
st.sidebar.header("Ask SafeSpace Bot")
...
if user_input:
    ...
    response = "I'm sorry, I don't understand your question. Please try rephrasing or contact us at <a href='#contact'>our contact section</a> for more help."
    for faq in faqs:
        if any(keyword in user_input_lower for keyword in faq["keywords"]):
            response = faq["answer"]
            break
